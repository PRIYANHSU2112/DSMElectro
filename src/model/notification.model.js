...
const ORDER_MESSAGES = {
  ORDER_PLACED: {
    title: "Order Placed! 🎉",
    body:  (orderId) => `Your order #${orderId} has been placed successfully. We'll keep you updated!`,
  },
  ORDER_SHIPPED: {
    title: "Order Shipped! 🚚",
    body:  (orderId) => `Your order #${orderId} is on its way! Track it in the app.`,
  },
  ORDER_ARRIVING: {
  ...
  ORDER_DELIVERED: {
    title: "Delivered! ✅",
    body:  (orderId) => `Your order #${orderId} has been delivered. Enjoy your purchase!`,
  },
  ORDER_CANCELLED: {
    title: "Order Cancelled",
    body:  (orderId) => `Your order #${orderId} has been cancelled. Contact support if this was a mistake.`,
  },
};
...
export default class NotificationService {
  ...
  static async sendToUser(userId, { title, message, type = "GENERAL", orderId = null }) {
    const user = await userModel.findById(userId).select("fcmToken").lean();
  ...
  static async sendToAllUsers({ title, message, type = "GENERAL" }) {
    ...
    for (let i = 0; i < tokens.length; i += 500) {
      ...
      NotificationService._sendPush({ tokens: batch, title, body: message, data: { type } });
    }
  }
